...
#define MARCHSTEPS 94
#define MARCHSTEPSREFLECTION 30
#define DEBRISCOUNT 5
...
float mapDamageHigh( vec3 p ) {
    ...
    float ne = 0.;
    ne += smoothstep( 0.3, 1., 1.-distance( p, ep1.pos ) );
    ne += smoothstep( 0.3, 1., 1.-distance( p, ep2.pos ) );
    ne += smoothstep( 0.3, 1., 1.-distance( p, ep3.pos ) );
    ne += smoothstep( 0.3, 1., 1.-distance( p, ep4.pos ) );
    ne += smoothstep( 0.3, 1., 1.-distance( p, ep5.pos ) );
...
float calcAO( in vec3 pos, in vec3 nor ) {
	float occ = 0.0;
    for( int i=0; i<6; i++ ) {
        float h = 0.1 + 6.*float(i)/5.0;
        occ += (h-map( pos + h*nor ));
    }
    return clamp( 1.0 - .2*occ/8.0, 0.0, 1.0 );    
}
...
float matMarble( in vec3 pos, in vec3 nor ) {
    ...
    return .6 * (.12*hash(i)+.2*noise(pos*25.)+.12*noise(pos*7.));
}
...
vec2 shade( in vec3 pos, in vec3 nor, in float m, in float t, in bool reflection ) {
    ...
    if( m < .5 ) {
        ...
        } else if( pos.y > 13.5 && pos.y < 13.99 && abs( pos.x ) > 22.99 ) {
...
float renderExplosionDebris( const in vec3 ro, const in vec3 rd, in float maxdist, const in Explosion ep, inout vec3 nor, const in float time ) {
    float maxRadius = 30.*(time - ep.startTime - .025);
    float minRadius = 0.2 * maxRadius;
    if( maxRadius > 30. ) return maxdist;
    
    for( int i=0; i<DEBRISCOUNT; i++ ) {
        float id = hash(  ep.startTime+float(i) );
        vec3 dir = normalize( -1.+2.*vec3( id, hash(  ep.startTime+.5*float(i) ), hash(  ep.startTime+1.5*float(i) ) ) - vec3( 2.*sign(ep.pos.x), 0., 0.) );
        vec3 pos = ep.pos + dir*mix( minRadius, maxRadius, id ) + vec3(0.,-maxRadius*sin( maxRadius*0.005 ),0.);
...
void renderExplosionDust( const in vec3 ro, const in vec3 rd, in float dist, const in Explosion ep, inout vec2 col, const in float time ) {
    float maxRadius = 10.*(time - ep.startTime + .25);
    if( maxRadius > 40. ) return ;
    
    float dens = 0.;
    float ho = hash( ep.startTime ); // id of explosion
    float fade = pow( 2., -maxRadius*0.11-2.) ;
    vec2 down = vec2(sin((maxRadius*0.1+2.)*.05), 0.);
                     
	// intersect planes
    float d = -(ro.x - ep.pos.x )/rd.x;
    if( d > 0. ) {
        vec3 pos = ro+d*rd;
        float radius = distance( ep.pos.yz, pos.yz );
        if( radius < maxRadius  ) {
            float l = max( (dist-d+20.)/40., 0. ) 
                        			* fade 
                      				* abs( dot(gRd,vec3(1.,0.,0.)) )
                     				* (1.-smoothstep( 0.8*maxRadius, maxRadius, radius ));            
	        float excol = mix( col.x, 1., pow(1.-2.*texture2D( iChannel2,ho+(pos.yz-ep.pos.yz)*2.5/(maxRadius) + down, -100.0 ).x,3.) );               
    	    col.x = mix( col.x, excol, l);
            col.y += l;
        }
    }
    
    d = -(ro.z - ep.pos.z )/rd.z;
    if( d > 0. ) {
        vec3 pos = ro+d*rd;
        float radius = distance( ep.pos.yx, pos.yx );
        if( radius < maxRadius  ) {
            float l = max( (dist-d+20.)/40., 0. ) 
                        			* fade 
                      				* abs( dot(gRd,vec3(0.,0.,1.)) )
                     				* (1.-smoothstep( 0.8*maxRadius, maxRadius, radius ));
	        float excol = mix( col.x, 1., pow(1.-2.*texture2D( iChannel2,ho+(pos.yx-ep.pos.yx)*2.5/(maxRadius) + down, -100.0 ).x,3.) );               
    	    col.x = mix( col.x, excol, l);
            col.y += l;
        }
    }
...
vec3 render( const in vec3 ro, const in vec3 rd, in float time, const in float fog ) {
    ...
    col.y = 0.; 
    renderExplosionDust( ro, rd, t, ep1, col, time );
    renderExplosionDust( ro, rd, t, ep2, col, time );
    renderExplosionDust( ro, rd, t, ep3, col, time );
    renderExplosionDust( ro, rd, t, ep4, col, time );
    renderExplosionDust( ro, rd, t, ep5, col, time );
...
#define E1(a,b) t+=a;if( time >= t ){ep1.pos=b;ep1.startTime=t;}
#define E2(a,b) t+=a;if( time >= t ){ep2.pos=b;ep2.startTime=t;}
#define E3(a,b) t+=a;if( time >= t ){ep3.pos=b;ep3.startTime=t;}
#define E4(a,b) t+=a;if( time >= t ){ep4.pos=b;ep4.startTime=t;}
#define E5(a,b) t+=a;if( time >= t ){ep5.pos=b;ep5.startTime=t;}

void initExplosions( in float time ) {
	ep1.pos = ep2.pos = ep3.pos = ep4.pos = ep5.pos = vec3(-1000.);
    float t = 0.;
    ...
    E1(STT(58.),  vec3( 16., 3.9, 2.2 ) );
    E2(.2, vec3( 16., 5.4, 4.1 ) );
    E3(.3, vec3( 24., 6.3, 3.7 ) );
    E4(.5, vec3( 16., 4.8, 8.2 ) );
    E5(.7, vec3( 24., 5.7, 4.3 ) );
    E1(.1, vec3( 16., 1.9, 8.2 ) );
    E2(.2, vec3( 24., 5.4, -2.1 ) );
    
    t = 0.;
    E1(STT(66.),  vec3( 16., 3.9, 6.5 ) );
...
void getCamPath( const in float time, inout vec3 ro, inout vec3 ta, inout float fl, inout float fog ) {
    ...
    SCAM(STT(7.5), 0., 0.,  0.05, 1.5, 1.5, vec3( 18., 4., 11. ), vec3( 21.5, 4., 11.5 ),  vec3( 0., 5., -50. ), vec3( -4., 7., 0. ) ); 
    SCAM(STT(2.5), 0., 0.05, 0.1, 1.5, 1.5, vec3( 21.5, 4., 11.5 ), vec3( 21.5, 4., 11.5 ),  vec3( -4., 7., 0. ), vec3( -16., 7., 8. ) ); 
    SCAM(STT(4.), 0.,  0.1, 0.15, 1.5, 4.5, vec3( 21.5, 4., 11.5 ), vec3( 10., 4.25, 11.35 ),  vec3( -16., 7., 8. ), vec3( -16., 6., 8. ) ); 
	...
	SCAM(STT(4.), .45, 0.5,  0.65, 1.5, 1.2, vec3( -10., 5.25, -6. ), vec3( -12., 5.25, -9. ),  vec3( -15.5, 5.25, -7.3 ), vec3( -13.5, 6.25, 2.3 ) );     
    ...
    SCAM(STT(4.),  1., 0.8,  0.6, 1.5, 1.5, vec3( -26., 3.25, -6. ), vec3( -26., 3.25, -6. ),  vec3( -15.5, 6.25, -7.3 ), vec3( -15.5, 6.25, -7.3 ) );     
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    // letterbox
    if( abs(2.*fragCoord.y-iResolution.y) > iResolution.x * 420./1000. ) {
    ...
    col += render( ro, rd, time, fog );
    
    col *= vec3(1.9,2.1,1.9)/2.7;    
	col = col*0.9 + 0.1*col*col*(3.0-2.0*col);
	col *= 1.3*vec3(1.06,1.2,1.0);
	...
	// fade in
    col = mix( vec3(0.), col, clamp( time*.7, 0., 1. ) );
    col = mix( vec3(0.), col, clamp( abs(time-STT(12.)), 0., 1. ) );
    if( time < STT(33.5) ) col = mix( vec3(0.), col, clamp( (STT(33.5)-time-.5), 0., 1. ) );
    col = mix( vec3(0.), col, clamp( abs(time-STT(98.)), 0., 1. ) );
